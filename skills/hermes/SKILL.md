...
2. **Size and tailor.** From the study, select the scenario, run sizing to scale it to the project value, then tailor it. Record the result in the project management plan and the execution structure plan. The floor is the **minimal preset** (the four mandatory modules and their minimum required documents only), which composes with the lean profile for the lightest setup. Full procedure and preset in [references/scenarios-sizing-tailoring.md](references/scenarios-sizing-tailoring.md).
